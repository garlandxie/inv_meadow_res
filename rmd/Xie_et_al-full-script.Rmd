...
seed_mix_link <- "https://docs.google.com/spreadsheets/d/1A5CFyFKqYont4hAs_H03FIah2sVYp4eeU3kmtQdK2rA/edit?usp=sharing"

mw_seed_mix <- googlesheets4::read_sheet(
    seed_mix_link, 
    sheet = "Seed_mix"
    )
```
...
Clean the dataset so that I get all of the desirable species from seed mix 1 and 2 for Meadoway section 4.

```{r}
seed_mix_s4 <- mw_seed_mix %>%
  janitor::clean_names() %>%
  ...
  select(binom_latin, seed_mix_1, seed_mix_2, seed_mix_1_and_2)
...
sb_org_status <- sb_tidy %>%
  ...
  mutate(exotic_native = case_when(
   ...
   binom_latin == "Conyza_canadensis" ~ "N", 
...
\
Next, I'll visualize which species are exotic among the sampled sites.

```{r echo=FALSE, message=FALSE, warning=FALSE}
sb_org_status %>%
  group_by(
    season, site, treatment, plot,  
    binom_latin, exotic_native
    ) %>%
  summarize(abundance = sum(abund, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(exotic_native == "E", 
         season == "Spring",
         treatment == "TIL") %>%
  ggplot(aes(x = binom_latin, y = abundance)) + 
  geom_point() +
  facet_wrap(~site) +
  labs(
    title = "Exotic Species - Section 2 (Spring)", 
    x = NULL, 
    y = "Seedling Abundance", 
  ) + 
  coord_flip() + 
...
sb_org_status %>%
  ...
  facet_wrap(~site) +
  labs(
    title = "Exotic Species - Section 2 (Fall)", 
    x = NULL, 
    y = "Seedling Abundance", 
  ) + 
  coord_flip() + 
...
sb_org_status %>%
  ...
  facet_wrap(~site) +
  labs(
    title = "Exotic Species - Section 4 (Spring)", 
    x = NULL, 
    y = "Seedling Abundance", 
  ) + 
  coord_flip() + 
...
sb_org_status %>%
  ...
  summarize(abundance = sum(abund, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(exotic_native == "E", treatment == "RES") %>%
  
  # add a logarithm function to make the trends easier to trend
  # because of an extreme outlier (i.e., Verbascum thapsus)
  ggplot(aes(x = binom_latin, y = log(abundance))) + 
  geom_point() +
  facet_wrap(~site) +
  labs(
    title = "Exotic Species - Section 4 (Spring + Fall)", 
    x = NULL, 
    y = "log(Seedling Abundance)", 
  ) + 
  coord_flip() + 
  theme_bw() 
...
sb_org_status %>%
  ...
  facet_wrap(~site) +
  labs(
    title = "Exotic Species - Section 4 (Fall)", 
    x = NULL, 
    y = "Seedling Abundance", 
  ) + 
  coord_flip() + 
...
\
Then, I'll visualize which species are native among the sampled sites.
...
sb_org_status %>%
  ...
  facet_wrap(~site) +
  labs(
    title = "Native Species - Section 2 (Spring)", 
    x = NULL, 
    y = "Seedling Abundance", 
  ) + 
  coord_flip() + 
...
sb_org_status %>%
  ...
  facet_wrap(~site) +
  labs(
    title = "Native Species - Section 2 (Fall)", 
    x = NULL, 
    y = "Seedling Abundance", 
  ) + 
  coord_flip() + 
...
sb_org_status %>%
  ...
  facet_wrap(~site) +
  labs(
    title = "Native Species - Section 4 (Spring)", 
    x = NULL, 
    y = "Seedling Abundance", 
  ) + 
  coord_flip() + 
...
sb_org_status %>%
  ...
  facet_wrap(~site) +
  labs(
    title = "Native Species - Section 4 (Fall)", 
    x = NULL, 
    y = "Seedling Abundance", 
  ) + 
  coord_flip() + 
...
It would also be interesting to see how many native species are in the TRCA seed mix fro section 4.
...
sb_org_status %>%
  ...
  ggplot(
    aes(
      x = binom_latin, 
      y = abundance, 
      col = seed_mix_1_and_2)
    ) + 
  geom_point() +
  scale_color_discrete(guide="none") + 
  facet_wrap(~site) +
  labs(
    title = "Native Species - Section 4 (Spring + Fall)", 
    x = NULL, 
    y = "Seedling Abundance", 
  ) + 
  coord_flip() + 
...
exotic_richness <- 
  ...
  # just get the exotic species for this calculation
  filter(exotic_native == "E") %>%
  rename(exotic_status = exotic_native) %>%
  
  # first, get a list of exotic species (and their abundance) in each plot
  group_by(site, treatment, plot, spp_code) %>%
  summarize(abundance = sum(abund, na.rm = TRUE)) %>%
  ungroup() %>%
  
  # then, calculate number of exotic species in each plot
  group_by(site, treatment, plot) %>%
  summarize(exotic_richness = n_distinct(spp_code)) %>%
  ungroup()

exotic_richness %>%
  ggplot(aes(x = site, y = exotic_richness, fill = treatment)) +
    geom_boxplot() + 
    scale_fill_discrete(labels = c("Undisturbed", "Heavily-Tilled")) +
    labs(
      title = "Seed Bank (Spring + Fall Sampling)",
      x = NULL, 
      y = "Exotic Richness") + 
    theme_bw()
```
...
exotic_abundance %>%
  ggplot(aes(x = site, y = abundance, fill = treatment)) +
    geom_boxplot() + 
    scale_fill_discrete(labels = c("Undisturbed", "Heavily-Tilled")) +
    labs(
      title = "Seed Bank (Spring + Fall Sampling)",
