...
# rank-abundance curve: BNSH ---------------------------------------------------

# Note: BNSH and DAVE overlap a bit for the NMDS plots
# So plot rank abundance curves for BNSH (for now)
...
# current gamma distribution fit results in non-convergence for glm's 
# ignore for now.. 
rad_bnsh <- vegan::radfit(bnsh_comm, family = Gamma)
plot(rad_bnsh)
...
(RA_BNSH <- bnsh_comm %>%
    ...
    geom_line() + 
    geom_point() + 
    gghighlight(species %in% c("MOFI", "SIPE", "SOSP", "FERU"))  + 
    geom_text_repel(aes(label = species)) + 
    labs(title = "BNSH") + 
    theme_bw()
)
